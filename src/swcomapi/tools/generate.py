...
def enum_reference():
    """`docs/reference/enums.md`: every enumeration, with its link.

    Built from the data already generated into the package, so this needs no
    SOLIDWORKS and no type libraries - just a previous `generate`. 1,434
    enumerations is not a table to keep up to date by hand.

    Returns the Markdown as a str.
    """
    ...
    names = sorted(ENUMS)
    constants = sum(len(ENUMS[name]) for name in names)

    by_library = {}
    for name in names:
        by_library.setdefault(ENUM_LIBRARY[name], []).append(name)

    lines = [
        "# Enumerations",
        "",
        f"Every enumeration in the SOLIDWORKS {SOLIDWORKS_YEAR} API: "
        f"**{len(names):,} of them, {constants:,} constants**, read from the "
        "type libraries of the installed release.",
        "",
        "```python",
    ...
    for library in sorted(by_library):
        owned = by_library[library]
        lines.append(
            f"| {library} | {len(owned):,} | "
            f"{sum(len(ENUMS[name]) for name in owned):,} |"
        )
    ...
    for name in names:
        url = doclinks.link(name)
        link = f"[help]({url})" if url else "-"
        lines.append(
            f"| `{name}` | {len(ENUMS[name])} | {ENUM_LIBRARY[name]} | {link} |"
        )
    lines.append("")
    return "\n".join(lines)


def write_enum_reference(root=None):
    """Write `docs/reference/enums.md`. Returns ``(path, size)``."""
    # package_root() is <repo>/src/swcomapi, so the repository is two up.
    repository = os.path.dirname(os.path.dirname(package_root()))
    target = os.path.join(root or repository, "docs", "reference", "enums.md")
